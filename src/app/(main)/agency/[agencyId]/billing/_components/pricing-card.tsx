...
const PricingCard = ({ features, buttonCta, title, description, amt, duration, highlightTitle, highlightDescription, customerId, prices, planExists }: Props) => {
                ...
                <CardContent>
                    <ul>
                        {features.map((feature, index) => (
                            <li key={index} className="list-disc ml-4 text-muted-foreground">
                                {feature}
                            </li>
                        ))}
                    </ul>
