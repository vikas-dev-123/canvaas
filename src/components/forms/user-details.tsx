...
const UserDetails = ({ id, type, userData, subAccounts }: Props) => {
    ...
    const form = useForm<z.infer<typeof userDataSchema>>({
        ...
        defaultValues: {
            ...
            avatarUrl: userData?.avatarUrl ? userData?.avatarUrl : data?.user?.avatarUrl,
    ...
    const handleChangePermission = async (subAccountId: string, value: boolean, permissionId: string | undefined) => {
        if (!data?.user?.email) return;
        setLoadingPermissions(true);
        const response = await changeUserPermission(permissionId ? permissionId : v4(), data.user.email, subAccountId, value);
    ...
    const onSubmit = async (values: z.infer<typeof userDataSchema>) => {
        ...
        if (userData || data?.user) {
            ...
            authUserData?.Agency?.SubAccount.filter((subacc) => authUserData.Permissions.find((p) => p.subAccountId === subacc.id && p.access)).forEach(async (subaccount) => {
                await saveActivityLogsNotification({
                    agencyId: undefined,
                    ...
                    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                        ...
                        <FormField
                            ...
                            render={({ field }) => (
                                <FormItem className="flex-1">
                                    ...
                                    <FormControl>
                                        <Input readOnly={userData?.role === "AGENCY_OWNER" || form.formState.isSubmitting} placeholder="Email" {...field} />
                                    </FormControl>
                                    <FormMessage />
                                </FormItem>
                            )}
                        ...
                        {authUserData?.role === "AGENCY_OWNER" && (
                            <div>
                                ...
                                <FormDescription className="mb-4">You can give Sub Account access to team member by turning on access control for each Sub Account. This is only visible to agency owners</FormDescription>
                                    ...
                                    {subAccounts?.map((subAccount) => {
                                        ...
                                        return (
                                            <div key={subAccount.id} className="flex items-center justify-between rounded-lg border p-4">
                                                ...
                                                <Switch disabled={loadingPermissions} checked={subAccountPermissionsDetails?.access} onCheckedChange={(permission) => handleChangePermission(subAccount.id, permission, subAccountPermissionsDetails?.id)} />
                                            </div>
                                        );
                                    })}
                                </div>
